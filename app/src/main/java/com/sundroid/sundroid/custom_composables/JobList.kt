    ...
    Card(
        ...
        Row( modifier = Modifier.fillMaxWidth(),
            ...
            Column() {
                ...
                formatCurrency( job.amount.toString())?.let { Text(text = it, modifier= modifier.width(100.dp), textAlign = TextAlign.Center, style = MaterialTheme.typography.titleMedium) }
            }
        }
...
fun StatusView(icon : Int, state: Boolean){
    Image(painter = painterResource(id = icon), contentDescription = "Hello", colorFilter = getColorFilter(state) ,modifier = Modifier
        .padding(5.dp)
        .width(45.dp))
}
@Preview
